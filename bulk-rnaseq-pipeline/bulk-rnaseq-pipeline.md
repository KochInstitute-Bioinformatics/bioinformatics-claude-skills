...
- *Consolidated client report* — **one self-contained HTML that embeds the actual results** (figures + tables from all three stages) for handing to the scientist. Build it as a separate Rmd (`{...}_report.Rmd`) that **loads the saved outputs** (`import_checkpoint.rds`, `deg_checkpoint.rds`, `*_GSEA_fgsea.xlsx`, `*_ORA_clusterProfiler.xlsx`) and re-plots — do NOT re-run DESeq. Use `echo=FALSE` globally (hide all code), narrative section intros in plain language (define "up = higher in <test>"), `output: html_document` with `toc_float`, `theme: flatly`, `df_print: kable`, `self_contained: true`. Render with a clean `output_file` like `{PROJECT}_Results_Report.html`. Verify it embeds figures as base64 with zero external `src` (truly portable single file).
  - **Interactive volcano + DEG table:** a `plotly` volcano (hover for gene name) alongside a `DT::datatable()` of the same expressed-protein-coding-MPG universe, both built from plain data frames (no `crosstalk` linkage — a linked-slider version was tried and then deliberately removed per user feedback; keep this section to a standalone plot + table). Sort the DEG table by `padj` ascending by default via DT's `order` option, and give it `extensions = "Buttons"` with `buttons = c("copy", "csv", "excel")` so the client can export exactly what they've searched/sorted to. Still a single portable HTML file (`self_contained: true` still applies; verify no external `src` as before).
  - **Interactive GSEA/ORA tables:** replace static `kable` GSEA/ORA tables with one `DT::datatable()` per collection (GSEA) or per collection×direction (ORA), same `Buttons` export extension as the DEG table. Show **all** significant sets (`padj < 0.05`), not a fixed top-N — a hardcoded top-15/20 cap defeats the point of an interactive, searchable table when a collection like C5 GO:BP can have hundreds of hits. Header each table with a count like "N significant / M tested" for context. Rendering multiple `DT` widgets from a loop over sheet names in one chunk requires `results='asis'` and returning `htmltools::tagList(lapply(sheets, \(sh) htmltools::tagList(htmltools::h4(...), DT::datatable(...))))` as the chunk's final value — a bare `for` loop with `print(knitr::kable(...))` only ever shows the last widget.
